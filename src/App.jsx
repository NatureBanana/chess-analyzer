...
function winRate(games) {
  return games.length?Math.round(games.filter(g=>g.result==="win").length/games.length*100):null;
}
...
function eloBrackets(games) {
  return [["<1000",0,1000],["1000-1199",1000,1200],["1200-1399",1200,1400],["1400-1599",1400,1600],["1600-1799",1600,1800],["1800-1999",1800,2000],["2000+",2000,9999]]
    .map(([label,mn,mx])=>{ const g=games.filter(x=>x.oppElo!=null&&x.oppElo>=mn&&x.oppElo<mx); return {label,games:g.length,winPct:g.length?Math.round(g.filter(x=>x.result==="win").length/g.length*100):0}; })
    .filter(b=>b.games>0);
}
...
function CompareMiniCard({p,accent,t}) {
  const wr=p.games.length?Math.round(p.games.filter(g=>g.result==="win").length/p.games.length*100):null;
  return <Card t={t} style={{flex:1,minWidth:180}}>
    ...
    <div style={{display:"flex",justifyContent:"space-between",padding:"6px 0",fontSize:13}}><span style={{color:t.textDim}}>Win rate</span><span style={{color:accent,fontWeight:700}}>{wr??"—"}{p.games.length?"%":""}</span></div>
  </Card>;
}
...
// ── Compare Tab ───────────────────────────────────────────────────────────────
function CompareTab({p1,p2,l1,l2,p2In,setP2In,loadP2,e2,t}) {
  const tip=(props)=><ChartTip {...props} t={t}/>;
  ...
  if (!p2&&!l2) return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    <div style={{textAlign:"center",padding:"24px 0 12px",color:t.textDim}}>
      ...
      <div style={{fontFamily:t.headingFont,fontSize:20,color:t.textMid,marginBottom:6}}>Player Comparison</div>
      <div style={{fontSize:13}}>Load a second player to compare stats and direct games</div>
    </div>
    {form}
  </div>;
  if (l1||(l2&&!p2)) return <Sk h={300}/>;
  if (!p1||!p2) return null;
  const norm=(v,mx)=>Math.round((v/Math.max(mx,1))*100);
  const bestRating = stats => Math.max(...["rapid","blitz","bullet","daily"].map(tc=>getRating(stats,tc).last||0));
  const p1r=bestRating(p1.stats), p2r=bestRating(p2.stats);
  const p1w=winRate(p1.games), p2w=winRate(p2.games);
  const p1pz=p1.stats?.tactics?.highest?.rating||0, p2pz=p2.stats?.tactics?.highest?.rating||0;
  const p1Elos=p1.games.filter(g=>g.oppElo).map(g=>g.oppElo), p2Elos=p2.games.filter(g=>g.oppElo).map(g=>g.oppElo);
  const ao1=p1Elos.length?Math.round(p1Elos.reduce((a,b)=>a+b,0)/p1Elos.length):0, ao2=p2Elos.length?Math.round(p2Elos.reduce((a,b)=>a+b,0)/p2Elos.length):0;
  const d1=uniqueNamedOpenings(p1.games), d2=uniqueNamedOpenings(p2.games);
  const radar=[{subject:"Win%",[p1.profile.username]:p1w??0,[p2.profile.username]:p2w??0},{subject:"Rating",[p1.profile.username]:norm(p1r,Math.max(p1r,p2r)),[p2.profile.username]:norm(p2r,Math.max(p1r,p2r))},{subject:"Puzzle",[p1.profile.username]:norm(p1pz,Math.max(p1pz,p2pz)),[p2.profile.username]:norm(p2pz,Math.max(p1pz,p2pz))},{subject:"Avg Opp",[p1.profile.username]:norm(ao1,Math.max(ao1,ao2)),[p2.profile.username]:norm(ao2,Math.max(ao1,ao2))},{subject:"Diversity",[p1.profile.username]:norm(d1,Math.max(d1,d2)),[p2.profile.username]:norm(d2,Math.max(d1,d2))}];
  const openingKey = o => `${o.eco&&o.eco!=="?"?o.eco:""}|${o.opening.toLowerCase().replace(/[^a-z0-9]+/g," ")}`.trim();
  const p2ByOpening = new Map(aggOpenings(p2.games).map(o=>[openingKey(o),o]));
  const shared=aggOpenings(p1.games).map(o=>({p1:o,p2:p2ByOpening.get(openingKey(o))})).filter(x=>x.p2).sort((a,b)=>(b.p1.games+b.p2.games)-(a.p1.games+a.p2.games)).slice(0,5).map(({p1:o,p2:o2})=>({opening:o.opening.length>18?o.opening.slice(0,16)+"…":o.opening,[p1.profile.username]:o.winPct,[p2.profile.username]:o2.winPct}));
  const h2h=p1.games.filter(g=>String(g.opponent||"").toLowerCase()===p2.profile.username.toLowerCase());
  const hWins=h2h.filter(g=>g.result==="win").length, hLosses=h2h.filter(g=>g.result==="loss").length, hDraws=h2h.filter(g=>g.result==="draw").length;
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    {form}
    {/* VS section */}
    ...
    <Card t={t}><SecTitle t={t} sub="Games found in the selected range">Direct Games</SecTitle>
      {h2h.length?<div style={{display:"flex",gap:18,flexWrap:"wrap",fontSize:14}}>
        <span style={{color:t.win,fontWeight:700}}>{hWins} wins</span>
        <span style={{color:t.loss,fontWeight:700}}>{hLosses} losses</span>
        <span style={{color:t.draw,fontWeight:700}}>{hDraws} draws</span>
        <span style={{color:t.textDim}}>{h2h.length} total</span>
      </div>:<div style={{color:t.textDim,fontSize:13}}>No direct games found between these players in this range.</div>}
    </Card>
    ...
    <Card t={t}><SecTitle t={t}>Shared Opening Win%</SecTitle>
      {shared.length?<ResponsiveContainer width="100%" height={200}>
      ...
      </ResponsiveContainer>:<div style={{color:t.textDim,fontSize:13,padding:"18px 0"}}>No shared openings found in this range.</div>}
    </Card>
  </div>;
}
...
function setHash(user, sub, other) {
  const path = sub ? `/${user}/${sub}${other?`/${other}`:""}` : user ? `/${user}` : "";
...
export default function App() {
  ...
  const doLoad1 = async (username, mo, opts={}) => {
    const u = (username||p1In).trim().toLowerCase();
    if (!u) return null;
    const m = mo !== undefined ? mo : monthsRef.current;
    const loadId = ++p1LoadId.current;
    const clearCompare = opts.clearCompare ?? true;
    const samePlayer = p1?.profile?.username?.toLowerCase() === u;
    setL1(true); setE1(null);
    if (!samePlayer) setP1(null);
    if (clearCompare) { p2LoadId.current++; setP2(null); setP2In(""); setE2(null); }
    try {
      const data = await loadPlayer(u, m);
      if (loadId === p1LoadId.current) setP1(data);
      return data;
    }
    catch(e) { if (loadId === p1LoadId.current) setE1(e.message||"Failed to load"); return null; }
  ...
  const changeMonths = (m) => {
    ...
    setMonths(m);
    if (p1) doLoad1(p1.profile.username, m, { clearCompare:false });
    if (p2) load2(p2.profile.username, m, { updateHash:false });
  };

  const load2=async(username, mo, opts={})=>{
    const u = (username||p2In).trim().toLowerCase();
    if(!u)return;
    const m = mo !== undefined ? mo : monthsRef.current;
    const loadId = ++p2LoadId.current;
    const samePlayer = p2?.profile?.username?.toLowerCase() === u;
    setL2(true); setE2(null);
    if (!samePlayer) setP2(null);
    try{
      const data = await loadPlayer(u, m);
      if (loadId === p2LoadId.current) {
        setP2(data);
        setP2In(data.profile.username || u);
        if ((opts.updateHash ?? true) && p1) setHash(p1.profile.username, "compare", data.profile.username || u);
      }
    }
    catch(e){
      if (loadId === p2LoadId.current) setE2(e.message||"Failed to load comparison player");
  ...
  useEffect(()=>{
    const loadFromHash = () => {
      ...
      if (!user) return;
      const isCompare = sub==="compare";
      setP1In(user);
      if (sub==="card") setTab(5);
      if (isCompare) { setTab(4); if (other) setP2In(other); }
      doLoad1(user, undefined, { clearCompare: !isCompare }).then(()=>{
        if (isCompare&&other) load2(other, undefined, { updateHash:false });
      });
    };
    loadFromHash();
    ...
    // Hash routing is a mount/subscription effect; loaders receive explicit usernames from the URL.
  ...
  return <div style={{minHeight:"100vh",position:"relative"}}>
    ...
    <div style={{position:"relative",zIndex:1,maxWidth:960,margin:"0 auto",padding:"0 16px 80px"}}>
      ...
      {(p1||l1)&&<PageTransition keyVal={tab}>
        ...
        {tab===4&&<CompareTab p1={p1} p2={p2} l1={l1} l2={l2} p2In={p2In} setP2In={setP2In} loadP2={load2} e2={e2} t={t}/>}
