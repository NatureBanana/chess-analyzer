...
function parsePGNGame(pgn, user, game={}) {
  ...
  return { ...openingInfo, eco:tags.ECO||"?", color, result, oppElo:(!oppElo||isNaN(oppElo))?null:oppElo, timeControl, date:dateStr, endTime:game.end_time||0, opponent:color==="white"?b:w, url:game.url||tags.Link||null };
}
...
export async function loadPlayer(user, months=3, { fetchImpl } = {}) {
  ...
  const archiveData = await Promise.all(urls.map(u => fetchArchiveGames(u, user, { fetchImpl })));
